...
description: See how Neuro sends actions.
banner:
  content: |
...
- `id` - a string that identifies this specific execution. You'll need to use this ID when returning an [action result](/neuro-sdk-docs/base/actions/result).
- `name` - the name of the action Neuro wants to execute. During [action forces](/neuro-sdk-docs/base/actions/force), this will influence whether or not Neuro retries the entire action force.
...
- Neuro will be waiting for the action result before sending another action. For that reason, action results should be about validation, not actual results. [See more at the action result page](/neuro-sdk-docs/base/actions/result)
